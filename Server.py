...
class Server(object):
    ...
    def check_decision(self, block, difficulty):
        dump = json.dumps(block)
        self.sha1.update(dump.encode('utf-8'))
        hash = self.sha1.hexdigest()
        digit = convert_base(hash, 10, 16)
        print(digit)
        return difficulty > int(digit)

    def prepare_message(self, msg):
        transaction = json.loads(msg)
        if 'block_num' in transaction:
            del transaction['block_num']
            if transaction['difficulty'] == self.difficulty and self.check_decision(transaction, transaction['difficulty']):
                print('found block by another NODE')
                print(transaction)
                self.foundBlocks[transaction['block_num']] = True
            else:
                print('bad block: ')
                print(transaction)
                logerr('bad block: ' + str(int(self.check_decision(transaction, transaction['difficulty']))))
        else:
            # check sign
            message = {}
    ...
    def create_block(self):
        ...
        while True:
            for offset in range(0, 100):
                ...
                if self.check_decision(block, self.difficulty):
